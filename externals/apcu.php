...
$VERSION='$Id: apcu.php 828 2014-10-21 00:18:51Z julian $';
...
body,p,td,th,input,submit { font-size:0.8em; font-family: 'Ubuntu', sans-serif; font-size: 10px;}
  ...
  echo
	...
	if ($list) {
      ...
      if(!$MYREQUEST['SEARCH'] || preg_match($MYREQUEST['SEARCH'], $entry[$fieldname]) != 0) {
		...
		if ($sh == $MYREQUEST["SH"]) {
			echo '<tr>';
	echo '<td colspan="8" bgcolor="#DDDDDD" style="border-top: 1px solid #666699; border-bottom: 1px solid #666699;"><pre style="white-space: pre-wrap; white-space: -moz-pre-wrap; white-space: -pre-wrap; white-space: -o-pre-wrap; word-wrap: break-word;">'.htmlentities(print_r(apcu_fetch($entry['info']), 1)).'</pre></td>';
			echo '</tr>';
		}
        $i++;
